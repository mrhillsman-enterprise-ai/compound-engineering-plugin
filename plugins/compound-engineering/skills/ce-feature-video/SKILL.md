...
```bash
mkdir -p .context/compound-engineering/ce-feature-video/[RUN_ID]/screenshots
mkdir -p .context/compound-engineering/ce-feature-video/[RUN_ID]/videos
```
...
agent-browser screenshot .context/compound-engineering/ce-feature-video/[RUN_ID]/screenshots/01-start.png
```
...
agent-browser screenshot .context/compound-engineering/ce-feature-video/[RUN_ID]/screenshots/02-navigate.png
```
...
agent-browser wait 1000
agent-browser screenshot .context/compound-engineering/ce-feature-video/[RUN_ID]/screenshots/03-feature.png
```

```bash
agent-browser wait 2000
agent-browser screenshot .context/compound-engineering/ce-feature-video/[RUN_ID]/screenshots/04-result.png
```
...
```bash
ffmpeg -y -framerate 0.5 -pattern_type glob -i ".context/compound-engineering/ce-feature-video/[RUN_ID]/screenshots/*.png" \
  -c:v libx264 -pix_fmt yuv420p -vf "scale=1280:-2" \
  ".context/compound-engineering/ce-feature-video/[RUN_ID]/videos/feature-demo.mp4"
```
...
- `.context/compound-engineering/ce-feature-video/[RUN_ID]/videos/feature-demo.mp4` (normal recording flow)
...
```bash
rm -r .context/compound-engineering/ce-feature-video/[RUN_ID]
```

**If in record-only mode or upload failed**, remove only the screenshots but preserve the video so the user can upload later:

```bash
rm -r .context/compound-engineering/ce-feature-video/[RUN_ID]/screenshots
```
...
/ce-feature-video 847

# Record with custom base URL
/ce-feature-video 847 http://localhost:5000

# Record for staging environment
/ce-feature-video current https://staging.example.com
```
